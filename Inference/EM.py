...
def calc(w2cm, label_set):
    w2q = {}
    for w in w2cm:
        total = 0.0
        for label in label_set:
            total += w2cm[w][label][label]
        w2q[w] = total / len(label_set)
    return w2q

def infer(result,workerQualityList):
    e2wl,w2el,label_set = gete2wlandw2el(result) # generate structures to pass into EM
    iterations = 20 # EM iteration number
    initquality = 0.7
    e2lpd, w2cm = EM(e2wl,w2el,label_set,workerQualityList).Run(iterations)
    return get_e2t(e2lpd), calc(w2cm, label_set)
